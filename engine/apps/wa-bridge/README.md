...
Instantiates **named** agents from the campus core (`engine/apps/server`) into a WorkAdventure room using the same join path as a human (anonymous JWT → WebSocket → `JoinRoom`).
